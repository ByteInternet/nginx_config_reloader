from nginx_config_reloader import as_unprivileged_user, UNPRIVILEGED_GID, UNPRIVILEGED_UID
from tests.testcase import TestCase


class TestAsUnprivilegedUser(TestCase):
    def setUp(self):
        self.setgid = self.set_up_patch(
            'nginx_config_reloader.os.setgid'
        )
        self.setuid = self.set_up_patch(
            'nginx_config_reloader.os.setuid'
        )
